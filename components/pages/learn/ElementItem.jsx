...
export default function ElementItem({ element }) {
  ...
  const href = `/learn/${element.label}`

  return (
    <Link href={href} className="group relative overflow-hidden rounded-2xl border border-border/60 bg-surface p-6 transition-colors hover:border-primary/30">
      {element.img && (
        <div
          className="
  ...
  "
    ...
    "
      ...
      <div className="relative z-10">
        <span className="inline-flex h-10 w-10 items-center justify-center rounded-full bg-primary text-primary-foreground lg:bg-primary/10 lg:text-primary lg:group-hover:bg-primary lg:group-hover:text-primary-foreground">
          {Icon === "water" ? (
        ...
        <p className="text-sm italic text-primary">{element.sanskrit}</p>
